$(function (){
	let guessedLetters = document.querySelector('#guessedLetters') as HTMLInputElement,
		lastInput = document.querySelector('.search') as HTMLInputElement,
		instructionsBTN = document.querySelector('#instructions-btn') as HTMLInputElement,
		instructText = document.getElementById('instructions-content') as HTMLInputElement,
		hintContent = document.querySelector('#hint-content') as HTMLInputElement,
		avatarForm =  document.querySelector('#avatarForm') as HTMLInputElement;
	...
	let category: allowedCategories;
  type allowedCategories = 'tv' | 'movie';
  type allowedDifficulty = 'easy' | 'med' | 'hard';
	let difficulty: allowedDifficulty;
    ...
    avatar: number;
		...
		constructor (userName: string, userEmail: string, userPass: string, userAvatar: number, localData: StorageData) {
			this.name = userName;
		...
		setUserInfo () {
			if (localStorage.getItem('avatar') !== null) {
					//resolves the error Argument of type 'string | null' is not assignable to parameter of type 'string'. Type 'null' is not assignable to type 'string'
	...
	avatarForm.addEventListener('submit', function (e){
		...
		if (document.querySelector('input[name="avatar"]:checked') === null) {
			...
			userAvatar = +(document.querySelector('input[name="avatar"]:checked') as HTMLInputElement).id;
			userName = (document.getElementById('name') as HTMLInputElement).value;
			userEmail = (document.getElementById('email') as HTMLInputElement).value;
			userPass = (document.getElementById('password') as HTMLInputElement).value;
			newUser = new Avatar(userName, userEmail, userPass, userAvatar, localData);
			newUser.setLocal();
			newUser.setUserInfo();
			newUser.showUser();
			...
			let formID = 'avatarForm';
			let avatarForm = <HTMLFormElement>document.getElementById(formID);
			avatarForm.reset()
			responsive();
	...
	if (localStorage.getItem('avatar') !== null) {
		//passing in blank values when instantiating object as we will get actual values later on
		newUser = new Avatar("userName", "userEmail", "userPass", 0, localData);
		newUser.setUserInfo();
		newUser.showUser();
		$('#register-btn').addClass('d-none');
	...
	let startGame:{
		...
		createLetters : function (){
			for (let i = 0; i <= this.word.length - 1; i++) {
				...
				(document.getElementById('letterContainer') as HTMLInputElement).appendChild(tag);
			}
	...
	let checker:{
		...
		checkInput     : function (){
			...
			if (this.input) {
				...
				else if (guessedLetters.innerText.includes(this.input) || this.correctLetters.includes(this.input)) {
		...
		/*When a user guesses a letter not in the film title, take that input and add it to the guessedLetters element with a dash in between each letter. If there are 7 letters in the guessedLetters element then display last try in the DOM. If the guessedLetters element is not at 8 letters yet, call the hang function. If the guessedLetters element reached 8 letters then run loseGame function*/
		wrongAnswer    : function (){
			guessedLetters.innerText += `${this.input} -`;
			if ((guessedLetters.innerText.match(/\w/gi) || "").length === 7) {
				$('.win').text('Last try!');
				$('.win').css('display', 'block');
			}

			(guessedLetters.innerText.match(/\w/gi) || "").length === 8 ? gamePlay.loseGame() : gamePlay.hang();
		}
	};

	/*depending on the user input error(for ex. too many characters or inputting non letters), change the text of the error message to the parameter passed in and display it on the DOM. remove it from the DOM with setTimeout.*/
	...
	let gamePlay:{
		...
		/*When a user guesses a letter incorrectly, check how many incorrect guesses there are(based on the length of guessedLetters). Change the source of the hangman image based on the counter we are on.*/
		hang          : function (){
			let counter = (guessedLetters.innerText.match(/\w/gi) || "").length;
			for (let i = 0; i < counter; i++) {
				this.image.src = `/images/${i + 1}.JPG`;
			}
		},

		/*When a user loses the game, the interval for the timer stops. Input element is removed from the DOM. Play the lose audio. If screen is wide, the hangman image will change to the GIF. If screen isn't wide then "you lost" text will be displayed. The totalScore will be subtracted by 5 points. If there is no local storage, the totalPoints will be added to session storage, else the totalpoints will be added to local storage. */
		loseGame      : function (){
		...
		/*if wide screen, add the poster image into the DOM and set the source of the image to the poster source we fetched from the database earlier. Else if smaller than wide screen, display win(or lost) message and replace the hangman image with the poster image from the database*/
		...
		/*After a user guesses a letter correctly, loop through all the Ptags(letters) and if all of the letters have a black color: clear interval timer. If window is wide, don't display win text, change the hangman image source to the winning GIF. Then display the title poster image. Increase the totalScore based on difficulty.If there is localstorage, set the totalScore to local storage. If not, set the score to sessionstorage. Remove input. */
		winChecker    : function (){
			let count = 0;
			for (let i = 0; i < startGame.word.length; i++) {
			...
			if (count === startGame.word.length) {
				...
				difficulty === 'easy' ? (totalScore += 5) : difficulty === 'med' ? (totalScore += 10) : (totalScore += 15);
				if (localStorage.getItem('avatar') !== null) {
					localStorage.setItem('score', JSON.stringify(totalScore));
				}
				else if (localStorage.getItem('avatar') === null) {
